...
[`mentionIndicators`](/api-reference/enriched-text-input#mentionindicators) prop, which defaults to
`['@']`:
...
programmatically with [`startMention(indicator)`](/api-reference/enriched-text-input#startmention).

## Inserting a mention with `setMention`

Once a mention is being edited - the user has typed an indicator, or you've
called `startMention` - you insert the finished mention with [`setMention`](/api-reference/enriched-text-input#setmention):
...
* [**`onStartMention`**](/api-reference/enriched-text-input#onstartmention) fires when a mention starts - a good moment to open your suggestion list.
* [**`onChangeMention`**](/api-reference/enriched-text-input#onchangemention) fires on every edit to the query, handing you the text
  typed after the indicator - use it to filter the list.
* [**`onEndMention`**](/api-reference/enriched-text-input#onendmention) fires when the mention stops being edited - the cursor
  moved away, or the query was closed - so you can dismiss the list.
* [**`onMentionDetected`**](/api-reference/enriched-text-input#onmentiondetected) fires when the cursor enters or leaves a created mention.
...
* [**`onMentionPress`**](/api-reference/enriched-text#onmentionpress) fires when the user presses a mention.
