...
On Android, when [`numberOfLines`](/api-reference/enriched-text#numberoflines) is set to a value higher than `1`, only the
`'tail'` value of [`ellipsizeMode`](/api-reference/enriched-text#ellipsizemode) works correctly. This matches React Native's
...
Custom context-menu items ([`contextMenuItems`](/api-reference/enriched-text-input#contextmenuitems)) require **iOS 16 or newer** on
...
The [`cursorColor`](/api-reference/enriched-text-input#cursorcolor) prop is applied on Android and Web. On iOS the caret and
selection color both follow the [`selectionColor`](/api-reference/enriched-text-input#selectioncolor) prop.
