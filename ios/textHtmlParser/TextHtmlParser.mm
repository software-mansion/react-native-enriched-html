...
- (void)applyProcessedStyles:(NSArray *_Nonnull)processedStyles {
  ...
  // Custom and inline styles collected during the first pass so applyStyling:
  // runs after paragraph visual attributes are in place. Custom (priority 1)
  // is applied before inline (priority 2). Each entry is @[style,
  // adjustedRange].
  NSMutableArray *pendingCustomApply = [NSMutableArray array];
  NSMutableArray *pendingInlineApply = [NSMutableArray array];

  NSMutableArray *paragraphStyles = [NSMutableArray array];
  NSMutableArray *nonParagraphStyles = [NSMutableArray array];
  for (NSArray *arr in processedStyles) {
    StyleBase *style = _view->stylesDict[arr[0]];
    if (style == nullptr)
      continue;
    if ([style isParagraph]) {
      [paragraphStyles addObject:arr];
    } else {
      [nonParagraphStyles addObject:arr];
    }
  }

  // HtmlParser emits close-tag order (inner before outer). Apply paragraph
  // styles first so ZWS insertions happen before inline meta is written.
  [paragraphStyles
      sortUsingComparator:^NSComparisonResult(NSArray *a, NSArray *b) {
        NSRange rangeA = [(StylePair *)a[1] rangeValue].rangeValue;
        NSRange rangeB = [(StylePair *)b[1] rangeValue].rangeValue;
        if (rangeA.location != rangeB.location) {
          return rangeA.location < rangeB.location ? NSOrderedAscending
                                                   : NSOrderedDescending;
        }
        return NSOrderedSame;
      }];

  for (NSArray *styleGroup in @[ paragraphStyles, nonParagraphStyles ]) {
    BOOL isParagraphPass = styleGroup == paragraphStyles;

    for (NSArray *arr in styleGroup) {
      ...
      // Use an adjusted range so that applyStyling covers any ZWS characters
      // that were just inserted by addSpacesIfNeededInHost:inRange:. Without
      // this, a style applied to an empty range {0,0} would call applyStyling
      // on {0,0} even after a ZWS was inserted.
  ...
  // Apply visual styling for custom styles, then inline styles.
  for (NSArray *entry in pendingCustomApply) {
